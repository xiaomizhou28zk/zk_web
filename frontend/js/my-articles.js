(function () {
  ...
  function showGuest() {
    ...
    listEl.innerHTML = '<p class="empty-tip">请先登录后查看我的文章。</p>';
          ...
          listEl.innerHTML = items
            .map(function (a) {
              ...
              return (
                ...
                '<button type="button" class="my-articles-vis-switch' +
                (visOn ? ' is-on' : '') +
                '" role="switch" aria-checked="' +
                visOn +
                '" data-id="' +
                a.id +
                '" aria-label="首页展示"></button>' +
                '</div>' +
                '<a href="' +
                escapeHtml(editHref) +
                '" class="my-articles-edit">编辑</a>' +
                '</div>'
              );
            })
            .join('');
